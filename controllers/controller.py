            ...
            for order_line in draft_order_lines:
                ...
                if(setting):
                    if(setting['show_line_subtotals_tax_selection'] == 'tax_excluded'):
                        ...
                        if(order_line.currency_id.name=="CLP"):
                            _logger.warning("order_line.currency_id.name 003")
                            _logger.warning(order_line.currency_id.name)
                            _logger.warning("unit_price 003")
                            ...
                            _logger.warning("unit_price_parted 003")
                            _logger.warning(unit_price_parted)
                            unit_price = int(unit_price)
                            _logger.warning("unit_price 004")
                            _logger.warning(unit_price_parted)
                            _logger.warning(unit_price)
                        else:
                            unit_price = ((float(order_line.price_unit) + (float(order_line.price_tax) / float(order_line.product_uom_qty)))) # / float(order_line.product_uom_qty)
                            unit_price_parted = str(unit_price).split(".")
                        ...
                        elif(order_line.currency_id.name=="CLP"):
                            _logger.warning("order_line.currency_id.name 006")
                            _logger.warning(order_line.currency_id.name)
                            _logger.warning("unit_price 006")
                            _logger.warning(order_line.price_unit)
                            unit_price = ((float(order_line.price_unit) + (float(order_line.price_tax) / float(order_line.product_uom_qty)))) # / float(order_line.product_uom_qty)
                            unit_price_parted = str(unit_price).split(".")
                            _logger.warning("unit_price_parted 006")
                            _logger.warning(unit_price_parted)
                            _logger.warning("unit_price 006")
                            _logger.warning(unit_price)
                            unit_price = int(unit_price)
